...
def run_korean_ai(user_text):
    ...
    nouns = [t.form for t in tokens if t.tag.startswith("NN")]  # pyright: ignore[reportAttributeAccessIssue]
    ...
    response = ollama.chat(
        model="anpigon/eeve-korean-10.8b", messages=[{"role": "user", "content": cleaned_text}]
    )
